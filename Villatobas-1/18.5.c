/* Ejercicio 18.5  (esNumero.c)
       Realiza un programa que lea una cadena de caracteres por teclado y
       compruebe si tal cadena es un número entero válido.
       Números enteros válidos son, por ejemplo: 73, 1, 0, -8, -1000
       Números no válidos son: 74.5, 91A4, pepe, --9

  ************** Función gets() *************************
  La función gets() nos permite leer por teclado una cadena de caracteres.
  Se utiliza de la siguiente forma:
    char cadena[30];
    gets(cadena);
  Los caracteres introducidos por el usuario, hasta que éste pulsa "intro",
  se almacenan en el array cadena.
  Se añade el terminador de cadena a los caracteres leídos.
  Esta función se considera "insegura" y, en general, no es conveniente usarla:
  imagínese que pasaría si el usuario introduce, por ejemplo, 35 caracteres.
  Como la función no comprueba que no se sobrepase la longitud del array, esto
  puede hacer que el programa funcione de forma incorrecta.
*/
