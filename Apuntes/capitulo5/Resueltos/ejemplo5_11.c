...
int introducirHora(){
    ...
    do {// bucle de para comprobar que la hora introducida es correcta
        ...
        printf("Introduzca la hora por pantalla en formato 24 horas \n"
               "Ejemplo 14:45 se introducirá 1445 \n");
        scanf(" %d",&hora);
        if (hora/100<0 || hora/100>24){
            printf("Hora introducida no válida\n");
            horaValida=0;
        }
        else if ( (hora %100)>59 ){
            printf("Minutos introducidos no válidos\n");
            horaValida=0;
        }
    }
    while (!horaValida);
...
int conversionHoraria(int x){
    int hora12=0;
    if (x/100>12){
        // Las horas a partir de las 12:00 se codifican como un número positivo
